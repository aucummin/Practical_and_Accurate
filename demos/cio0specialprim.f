c
c     File cio0specialprim.f: A sample FORTRAN program to illustrate
c                             the use of some of the routines to
c                             manage AIRES compressed output files.
c                             In this example the special primary
c                             records are taken into account.
c
c
c     ------------------------------------------------------------------
...
c
c       f77 cio0specialprim.f -L/myhome/aires/lib -lAires
c
...
c
      ...
      integer           idxspcode, idxspegy
      integer           idxspver, idxspnumber
c
      logical           get1, altrec
      integer*4         indata(30)
      double precision  fldata(30)
c
      integer           nciorec, nshowers, npart, ntrail
...
c
     ...
     +                             4, dtyp, irc)
c
c     Some indices for the "external primary particle" record:
c
      idxspcode    = crofieldindex(ciochann, 3, 'Particle code',
     +                             4, dtyp, irc)
      idxspegy     = crofieldindex(ciochann, 3, 'Energy',
     +                             4, dtyp, irc)
c
c     Some indices for the "special primary trialer" record:
c
      idxspver     = crofieldindex(ciochann, 4, 'Version of external',
     +                             4, dtyp, irc)
      idxspnumber  = crofieldindex(ciochann, 4, 'Total number',
     +                             4, dtyp, irc)
c
...
c
c       External primary particle record.
c
c       Examples of available fields.
c
c       Integer fields
c
c         indata(idxspcode)   ---- Code of special primary pcle.
c
c       Real fields
c
c         fldata(idxspegy)    ---- Energy of special primary (log).
c
c       ------------------------------------------------------------
c       PLACE HERE YOUR "EXTERNAL PRIMARY PARTICLE" PROCESSING CODE.
c
...
c
c       Special primary trailer record.
c
c       Examples of available fields.
c
c       Integer fields
c
c         indata(idxspver)    ---- Version of external module.
c
c       Real fields
c
c         fldata(idxspnumber) ---- Total number of primary pcles.
c
c       ----------------------------------------------------------
c       PLACE HERE YOUR "SPECIAL PRIMARY TRAILER" PROCESSING CODE.
c
