c
c     FILE: kernel3a.f                      Creation date: 10/JUL/1997.
...
c
c     Version "a": Standard longitudinal recording files are in effect.
c
c<---><---><---><---><---><---><--->*<---><---><---><---><---><---><--->
c<---><---><---><---><---><---><--->*<---><---><---><---><---><---><--->
c
...
c
          do iobl = minlol + 1, maxlol
            ...
            if (olfilesave(iobl)) saveinfile = .true.
          enddo
c
c         Saving the particle event in the longitudinal tracking file.
c         The logical variable "saveinfile" is true only when one or
c         more of the observing levels marked for saving were crossed.
c
          if (saveinfile) then
            call olevcrossave(ipcode, chpsta(i), oldlol, newlol,
     +                        fpsta(ixenergy, i))
          endif
c
        endif
...
c
        if (saveinfile) then
c
          egy1 = egylss * (obslevdepth(minlol + 1) - dptnow)
          egy2 = egylss * (obslevdepth(maxlol) - dptnow)
          egyf = max(egy1, egy2)
c
          call olevcrossave(ipcode, chpdata, oldlol, newlol,
     +                      newe + egyf)
c
        endif
c
...
c       atmosphere, or is completely away from the shower axis: It's a
c       lost particle. Marking for deletion.
c       lowe is turned to .false. to prevent that the calling
c       program will interpret the particle as a low-energetic one.
c
        wte = wt1 * newe
...
c<---><---><---><---><---><---><--->*<---><---><---><---><---><---><--->
c
      subroutine olevcrossave(pclecode, chpclerecord,
     +                        poldol, pnewol, firstegy)
c
c     Processing a particle that passed across one or more observing
c     levels.
c
...
c     =========
c
c     pclecode........ (input, integer) Particle code.
c     chpclerecord.... (input, character*(*)) Particle data.
c     poldol.......... (input, integer) First observing level crossed
c                      (Previous to the first if the particle is going
c                      downwards).
c     pnewol.......... (input, integer) Last observing level crossed.
c                      (Previous to the last if the particle is going
c                      upwards).
c     firstegy........ (input, double precision) Energy of the particle
c                      at the first crossed observing level (GeV).
c
c
      implicit none
c
...
c
      ...
      integer           poldol, pnewol
      double precision  firstegy
c
c     Declaration of shared data.
c
      include 'initcomm.f'
...
c     Declaration of internal variables and arrays.
c
      integer           cofile, buffitem
c
c     FIRST EXECUTABLE STATEMENT
c
      cofile = longiciofile
c
c     The data is saved only if the particle is among the ones selected
c     for saving into the longitudinal tracking file.
c
      if (allpclesave(pclecode, cofile)) then
c
c       Appending the record into the corresponding particle buffer.
c       If the buffer is full, ship the buffered particles first.
c
        if (npopbuff(cofile) .ge. opbuffsize) call pcleship2
c
c       Appending the record.
c
        buffitem         = npopbuff(cofile) + 1
        npopbuff(cofile) = buffitem
c
        opbuffrec(buffitem, cofile)     = chpclerecord
        opbuffiaux(1, buffitem, cofile) = poldol
        opbuffiaux(2, buffitem, cofile) = pnewol
        opbufffaux(1, buffitem, cofile) = firstegy
c
      endif
c
      return
...
c     --- End of routine olevcrossave
c
c<---><---><---><---><---><---><--->*<---><---><---><---><---><---><--->
c<---><---><---><---><---><---><--->*<---><---><---><---><---><---><--->
c
c     End of file 'kernel3a.f'
