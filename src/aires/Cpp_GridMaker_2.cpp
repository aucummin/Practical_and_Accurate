...
std::unordered_map<std::array<double, 4>, std::array<double, 17>, ArrayHash, ArrayEqual> grid;
...
extern "C" {
    ...
    void update_vector(int PID, double X1, double Y1, double Z1, double T1, double X2, double Y2, double Z2, double T2, double E1, double E2, double w, double ANTX, double ANTY, double ANTZ) {
        ...
        while(T1 < T2) {
            ...
            //if(fabs(cell_id[0]-87506.25)<0.01){
            //    if(fabs(cell_id[1]+6.25)<0.01){
            //        if(fabs(cell_id[2]-34568.75)<0.01){
            //            if(fabs(cell_id[3]+296392.546339508030)<0.01){
            ...
            it->second[5] += w;
            //double random_val = rand()/RAND_MAX;
            //if(random_val < w/it->second[5]){
            //    it->second[6] = (betax/c);
            //    it->second[7] = (betay/c);
            //    it->second[8] = (betaz/c);
            //    
            ...
            //welford on V calculated
            double delta_Vx = betax/c - it->second[6];
            double delta_Vy = betay/c - it->second[7];
            double delta_Vz = betaz/c - it->second[8];
            it->second[6] += delta_Vx*w/it->second[5];
            it->second[7] += delta_Vy*w/it->second[5];
            it->second[8] += delta_Vz*w/it->second[5];
            
            double delta2_Vx = betax/c - it->second[6];
            double delta2_Vy = betay/c - it->second[7];
            double delta2_Vz = betaz/c - it->second[8];

            it->second[14] += delta_Vx*delta2_Vx*w;
            it->second[15] += delta_Vy*delta2_Vy*w;
            it->second[16] += delta_Vz*delta2_Vz*w;
            
            
            //it->second[6] += w*(betax/c); //V (velocity)
            //it->second[7] += w*(betay/c);
            //it->second[8] += w*(betaz/c);
            ...
            it->second[9] += w*(mid_X);
            it->second[10] += w*(mid_Y);
            it->second[11] += w*(mid_Z);
            it->second[12] += w*(mid_T);
            it->second[13] += w*(dt_ij);
    ...
    void calculate_radio(){
        ...
        for(auto it = grid.cbegin(); it != grid.cend(); ++it){
            ...
            double N = it->second[5];
            double ave_Vx = it->second[6];
            double ave_Vy = it->second[7];
            double ave_Vz = it->second[8];
            double cell_x = it->second[9];
            double cell_y = it->second[10];
            double cell_z = it->second[11];
            double cell_t = it->second[12];
            double dt = it->second[13];
            ...
            double delta_Vx = sqrt(it->second[14]/N);
            double delta_Vy = sqrt(it->second[15]/N);
            double delta_Vz = sqrt(it->second[16]/N);
            
            std::default_random_engine generator;
            std::cauchy_distribution<double> distribution_Vx(ave_Vx,delta_Vx);
            std::cauchy_distribution<double> distribution_Vy(ave_Vy,delta_Vy);
            std::cauchy_distribution<double> distribution_Vz(ave_Vz,delta_Vz);
            //
            //std::normal_distribution<double> distribution_x(0,4);
            //std::normal_distribution<double> distribution_y(0,4);
            //std::normal_distribution<double> distribution_z(0,4);
            //
            //cell_x += distribution_x(generator);
            //cell_x += distribution_y(generator);
            //cell_x += distribution_z(generator);
	    ...
	    //remember that my Jx has c... could mess things up
            ...
            double denom = fabs(1.-auxb); //missing abs
