...
C          S. Ostapchenko, Nucl.Phys.Proc.Suppl. B151 (2006) 143       C
C----------------------------------------------------------------------C
...
C 26.12.2005 - (SJS) Adapted for AIRES.                                C
C 20.03.2006 - (SJS) Some minor changes regarding sizes of arrays.     C
C 09.05.2006 - (SJS) Commented out some debugging print instructions.  C
C                                                                      C
C                 last modification:  09.05.2006                       C
C                        Version qgsjet-II-03                          C
C=======================================================================
C=======================================================================
C       Adapted for AIRES by S. J. Sciutto. La Plata, 26/Dec/2005.     C
C       (Modifications marked with ++SJS).                             C
C=======================================================================
      subroutine qgset
c-----------------------------------------------------------------------------
...
c ++SJS: Allow controlling DEBUG level externally.
c      moniou=6             !output channel for debugging
c      debug=0              !debugging level 
...
c-------------------------------------------------
 ...
 100  format(' ',
     ...
     *     /,' ','|                                                  |',
     *     /,' ','| Link to AIRES by S. J. Sciutto, 26/Dec/2005.     |',
     *     /,' ','|                                                  |',
     *     /,' ','====================================================',
     *     /)
...
c check normalization
     ...
     * ,fp(icz),rr
      ...
      if(lcalc)then
c ++SJS: Writing read message always.
c       if(debug.ge.1)write (moniou,203)
       write (moniou,203)
       open(1,file='qgsdat-II-03',form='unformatted',status='old')
...
c--------------------------------------------------
c ++SJS: Writing initialization message always.
c      if(debug.ge.1)write (moniou,204)   
      write (moniou,204)   
...
c ++SJS: Writing write message always.
c      if(debug.ge.1)write (moniou,227)      
      write (moniou,227)      
...
c Nuclear cross sections
      ...
      if(lcalc)then
c ++SJS: Writing read message always.
c       if(debug.ge.1)write (moniou,228)       
       write (moniou,228)       
...
c ++SJS: Writing a message to moniou unit
       write (moniou,2281)
...
228   format(2x,'qgaini: nuclear cross sections readout from the file'
     *,' sectnu-II-03')
2281  format(2x,'qgaini: nuclear cross sections calculation')
...
C++SJS      dimension wk(3),wj(3),wz(3),wi(3)
      common /qgarr15/ fp(3),rq(3),cd(2,3)
      common /qgarr17/ dels,alfp,sigs,rr,r3p,g3p,alpd,delh
      common /qgarr19/ ahl(3)
      common /qgarr20/ spmax
...
c------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=207)
      dimension vpau0(iapmax),vtau0(iapmax),vpac(iapmax),vtac(iapmax)
C++SJS      dimension vpau0(iapmax),vtau0(iapmax),vpau(iapmax),vtau(iapmax)
C++SJS     *,vpac(iapmax),vtac(iapmax),vpac0(iapmax),vtac0(iapmax)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr7/  xa(iapmax,3),xb(iapmax,3),b
      common /qgarr16/ cc(2,3),iddp(iapmax),iddt(iapmax)
...
C++SJS      dimension xb(iapmax,3),fhard(3),vabs(2)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr16/ cc(2,3),iddp(iapmax),iddt(iapmax)
      common /qgarr43/ moniou
...
c-----------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
C++SJS     parameter(iapmax=207,npbmax=1000,npnmax=900,npmax=500,legmax=900)
      parameter(iapmax=207,npbmax=1000)
      parameter(npnmax=1000,npmax=5000,legmax=1000)
...
C++SJS     if(nrej.gt.100)then (Change suggested by DH)
      if(nrej.gt.5)then
       if(debug.ge.1)write (moniou,209)
       goto 1
      endif
...
c-----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
C++SJS      parameter(iapmax=207,npmax=500,levmax=20,legmax=900)
      parameter(iapmax=207,npmax=5000,levmax=20,legmax=1000)
...
C++SJS      nprin=npgen(gbi/gbs,0,npmax) (Change suggested by DH)
C++SJS      ntgin=npgen(gbi/gbs,0,npmax)
...
c-----------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      integer debug
C++SJS      parameter(iapmax=207,npbmax=1000,npnmax=900,npmax=500
C++SJS     *,legmax=900,njmax=50000)
      parameter(iapmax=207,npbmax=1000)
      parameter(npnmax=1000,npmax=5000,legmax=1000,njmax=50000)
...
C++SJS      parameter(iapmax=207,npbmax=1000,npnmax=900,npmax=500,legmax=900)
      parameter(iapmax=207,npbmax=1000)
      parameter(npnmax=1000,npmax=5000,legmax=1000)
C++SJS      dimension alpi(npnmax,iapmax),alti(npnmax,iapmax)
C++SJS     *,alpt(iapmax),altt(iapmax)
      common /qgarr1/  ia(2),icz,icp
      common /qgarr2/  scm,wp0,wm0
      common /qgarr6/  pi,bm,amws
      common /qgarr9/  iwp(iapmax),iwt(iapmax),lqa(iapmax),lqb(iapmax)
...
c define color connections for the 2nd emitted jet
     ...
     * .and.i.ne.nvb.or.iqq.eq.3.and.i.ne.nva.and.i.ne.nvb)then
      ...
      enddo
C++SJS      if(ebal(1).gt.1.d-5) write (*,*)'ebal',ebal,iqq,jqq,nva,nvb     
...
C++SJS      dimension wp(2),ic(2),ept(4),ep(4),ep1(4),ey(3),ey0(3)
...
c-------------------------------------------------------------------------------
      ...
      integer debug
      parameter(iapmax=207)
C++SJS      dimension wabs(28),wdd(28),wqel(28),wcoh(28),wtot(28)
