...
C----------------------------------------------------------------------C
C             last modification:  Feb. 18, 2004                        C
C                        Version qgsjet01c.f                           C
C----------------------------------------------------------------------C
C  modifications are marked by cdh
C=======================================================================
C       Adapted for AIRES by S. J. Sciutto. La Plata, 22/Apr/2005.     C
...
c-----------------------------------------------------------------------
      ...
      INTEGER DEBUG
      CHARACTER *7 TY
C      LOGICAL LCALC,LSECT         (++ SJS LSECT not used
      LOGICAL LCALC
********************************************
...
c-------------------------------------------------
 ...
 100    FORMAT(' ',
     ...
     *     /,' ','|                                                  |',
     *     /,' ','| last modification:  Apr 22, 2005  by  S. Sciutto |',
     *     /,' ','|               (version qgsjet01c.f)              |',
     *     /,' ','====================================================',
     *     /)
...
c PSBINT procedure
      ...
      DO 14 L=1,2
      K1=K+17*(M-1)+34*(L-1)
C     ++ SJS. CALL TO QGKDUMMY ADDED TO MODIFY OPTIMIZED CODE.
      CALL QGKDUMMY(K1)
      IF(K.LE.I.OR.K.EQ.2)THEN
        CSBORN(I,K1)=-80.D0
      ELSE
...
c-----------------------------------------------------------------------
        ...
        INTEGER DEBUG
        CHARACTER*2 TYQ
C        DIMENSION EP(4,2),EPT(4),EPT0(4),EP3(4),EPJ(4),EPJ1(4),EY(3),
C        (++ SJS EP and EPT0 not used
...
c-----------------------------------------------------------------------
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        INTEGER DEBUG
C        DIMENSION EP3(4),EP1(4),EP2(4),EY(3)
C        (++ SJS EP1, EP2 not used
...
c-----------------------------------------------------------------------
      ...
      END
C
      SUBROUTINE QGKDUMMY(K)
C
C     ++ SJS. THIS SUBROUTINE DOES NOTHING.
C     ADDED TO FIX g77 OPTIMIZATION BUG IN INITIALIZATION PHASE.
C
      INTEGER K
      RETURN
      END
