...
C
C.  May 2002        (S. J. Sciutto) Code modification within routine
C.                  DECPAR, after a recommendation from R. Engel.
C.                  Extended the use of shared variable Ndebug to control
C.                  printout of debug/info data.
C.
C.  February 2002   (S. J. Sciutto) Minor code change in GASDEV to allow
C.                  reproduction of random numbers when re-running an
C.                  application.
C.                  Minor change in routine SIGNUC_INI (agrument IA0 added).
C
C
C-----------------------------------------------------------------------
...
c------
      ...
      IF (1.-XJET .LT. FLOAT(2*NW)*XMIN)  THEN
         NBAD = NBAD + 1
         LXBAD = 1
         IF ((NBAD .LE. 100) .AND. (Ndebug .GT. 0)) THEN
 ...
 5       FORMAT(1X,'NBAD = ',I3,3X,'sqs = ',E10.3,
     &            3X, 'x_jet = ', F9.3, 3X, ' NW = ',I2)
         IF ((NBAD .eq. 100) .AND. (Ndebug .GT. 0)) THEN
           WRITE (6, *) 
...
C-----------------------------------------------------------------------
      ...
      WRITE(*,*) ' Last modified 02/May/2002.'
      CALL PAR_INI
      CALL JET_INI
      CALL ZSAMPLE_INI
      CALL BLOCK_INI
...
C...initialization of proton and pion tables
      ...
      DO KK=1,2
         ...
         IF (Ndebug .GT. 0) WRITE(6,'(2(/,1X,A,A))') 
...
C...low-energy interpolation with data-parametrizations
     ...
     &                  SIGTOT,SIGEL,SIGINEL,SIGDIF,SLOPE,RHO)
        ...
        IF (Ndebug .GT. 0) WRITE(6,'(3X,I2,1P,E12.3,0P,4F8.2,3F8.3)')
     &    JINT,SQS,PTmin,SIG_tot,SIG_inel,B_el,RHO,PS,PH
...
C-----------------------------------------------------------------------
     ...
     &    'SIB_SIGMA_HP: interpolation table not initialized.'
      ...
      if((j1.lt.1).or.(j1.ge.NSQS)) then
        IF (Ndebug .GT. 0) write (6,'(1x,a,i3,1p,e12.3)') 
...
C   SJS: Following code line changed by suggestion of R. Engel
