...
C.
C.  Nov 2006        (S. J. Sciutto) Minor change un GASDEV to allow
C.                  compilation with gfortran compiler.
C.
C.  May 2002        Extended the use of shared variable Ndebug to control
...
C=============================================================
      ...
      SUBROUTINE SIGNUC_INI (IA0,IA,E0)
...
C.  with A  between IA0 and IA.
C.  IA0 and IA MUST verify: 2 .LE. IA0 .LE. IA .LE. 56.
C.  ++SJS: In the original SIBYLL package, IA0 is 2 (hardwired).
...
C     DO JA=2,IA replaced by SJS, introducing argument IA0
      DO JA=IA0,IA
         ABEAM = FLOAT(JA)
...
C...Gaussian deviation
      SAVE
C
C   Definition of ISET changed to allow reproducing the same numbers
c   in case of system crash.                                (++ SJS ++)
C   ISET must be set to TRUE before using the generator for the first
C   time.
